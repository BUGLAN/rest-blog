...
<script>
  ...
  export default {
    ...
    mounted() {
      this.$axios.get('http://buglan.org/api/manage', {headers: {'Authorization': 'Bearer '+ this.getCookie('token')}})
        .then(response => {
    ...
    methods: {
      article_delete: function (id) {
        this.$axios.delete('http://buglan.org/api/article_operation', {params: {'id': id}}, {headers: {'Authorization': 'Bearer '+ this.getCookie('token')}})
          .then(response => {
      ...
      category_delete: function (id) {
        this.$axios.delete('http://buglan.org/api/category_operation', {params: {'id': id}}, {headers: {'Authorization': 'Bearer '+ this.getCookie('token')}})
          .then(response => {
            if (response.status === 200) {
              alert('删除成功')
              this.$router.go(0)
      ...
      tag_delete: function (id) {
        this.$axios.delete('http://buglan.org/api/tag_operation', {params: {'id': id}}, {headers: {'Authorization': 'Bearer '+ this.getCookie('token')}})
          .then(response => {
            if (response.status === 200) {
              alert('删除成功')
              this.$router.go(0)
