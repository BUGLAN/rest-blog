...
<script>
  import NavHeader from '@/components/index/NavHeader'
    export default {
      ...
      mounted(){
      this.$axios.get('http://buglan.org/api/article_titles', {headers: {'Authorization': 'Bearer '+ this.getCookie('token')}})
        .then(response => {
      ...
      methods: {
          submit: function(){
            this.$axios.post('http://buglan.org/api/new_category', {'category': this.category, 'articles': this.check_articles}, {headers: {'Authorization': 'Bearer '+ this.getCookie('token')}})
              .then(response => {
